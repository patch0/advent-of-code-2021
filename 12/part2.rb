class Part2
  def run
    next_step(%w[start]).flatten.count{|s| s == 'start'}
      # .compact.join(',').gsub('end,',"end\n")
  end

  def next_step(route)
    possibles = input.select { |link| link.include?(route.last) }

    twice_visited_small_caves = route.tally.select { |c, count| count == 2 && c =~ /[a-z]/ }.keys

    visited_small_caves = ['start']
    visited_small_caves += route.select { |c| c =~ /[a-z]/ } if twice_visited_small_caves.any?
